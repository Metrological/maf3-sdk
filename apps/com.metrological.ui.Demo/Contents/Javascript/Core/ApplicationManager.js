		...
		if (!template) {
			...
			switch (type) {
					...
					new Text({
						...
						events: {
							...
							navigate: function (event) {
								var currentItem = this.retrieve('current');
								if (currentItem && event.detail.direction === 'down') {
									getElementById(currentItem).element.navigate('down', [0, 0]);
								}
				...
				case 'fullscreen':
					...
					break;
				case 'dialog':
					if (data.conf.type === 'adult' && !ProfileManager.profile.hasPIN(data.conf.type)){
						ApplicationManager.fire(identifier, 'onDialogDone', { key: data.key, success: true, previousDialog: data.previousDialog });
						return;
					}
					if (data.conf.type === 'purchase' && data.conf && data.conf.data && data.conf.type === 'purchase') {
						id = 'payment-info';
						var cur = '';
						switch(data.conf.data.currency.toLowerCase()) {
							case 'eur':
								cur = '€';
								break;
							case 'usd':
						...
						data.id = id;
						data.conf.title = $_('PAYMENT');
						data.conf.message = data.conf.data.description;
						data.conf.price = data.conf.data.price;
						if (data.conf.data.ttl)
							data.conf.ttl = data.conf.data.ttl;
						if (data.conf.data.subscription)
							data.conf.subscription = data.conf.data.subscription;
						data.previousDialog = {
							id: data.previousDialog
						};
						data.conf.formatPrice = cur + ' ' + (parseInt(data.conf.data.price, 10) / 100);
						data.conf.formatPrice = (data.conf.formatPrice.indexOf('.') === -1) ? data.conf.formatPrice + ',00' : data.conf.formatPrice.replace('.', ',');
					}
					...
					switch (id) {
						...
						case 'twitter-login':
							...
							break;
						case 'payment-info':
							buttons.push({ value: '$payment-go', label: 'ORDER' });
							buttons.push({ value: '$cancel', label: 'CANCEL' });
							break;
						case 'payment-pin':
							buttons.push({ value: '$cancel', label: 'CANCEL' });
							break;
						case 'payment-done':
							buttons.push({ value: '$payment-done', label: 'CLOSE' });
							break;
					}
								...
								if (target && target.id && target.id.indexOf('button') > 0) {
									...
									switch (selectedValue) {
										...
										case '$profile-pin':
											...
											break;
										case '$order':
											ApplicationManager.fire(identifier, 'onDialogPaymentStart', { key: dialogKey, profile: data.conf.profile, previousDialog: data.id });
											break;
										case '$payment-pin':
											paymentData = data.conf;
											ApplicationManager.fire(identifier, 'onDialogPaymentPIN', { key: dialogKey, profile: data.conf.profile, previousDialog: data.id });
											break;
										case '$payment-go':
											...
											for (var property in confData) {
												if (!allowedFields.contains(property))
													delete confData[property];
											}
											profile.purchase(confData, function (err, result) {
												if (err)
													ApplicationManager.fire(identifier, 'onDialogPaymentFailed', { key: dialogKey, code: err.message + ' (code ' + err.code + ')' });
												else
													ApplicationManager.fire(identifier, 'onDialogDone', { key: dialogKey, success: true, previousDialog: confData.previousDialog, result: result, type: 'purchase' });
											});
											break;
										case '$payment-done':
											ApplicationManager.fire(identifier, 'onDialogDone', { key: dialogKey, status: data.conf.status, code: data.conf.code, result: data.conf.result });
											break;
										default:
					...
					var dialogMessage = new Text({
						id: '@' + type + '-message',
						label: messageLabel + (dialogConfig.formatPrice ? '<br /><br />' + dialogConfig.formatPrice : ''),
						styles: {
							width: '100%',
							paddingLeft: 10,
							paddingRight: 10,
...
widget.handleHostEvent = function (event) {
	...
	switch(event.subject) {
		...
		case 'onDialogProfileCreated':
			...
			break;
		case 'onDialogPaymentStart':
			data = event.getData();
			break;
		case 'onDialogPaymentPIN':
			data = event.getData();
			loadTemplate.call(this, { type: 'dialog', id: 'payment-pin', conf: { key: data.key, title: 'PAYMENT', id: paymentData.data.id, message: paymentData.message, price: paymentData.price, formatPrice: paymentData.formatPrice, adult: paymentData.data.adult, currency: paymentData.data.currency, timestamp: paymentData.data.timestamp, signature: paymentData.data.signature, ttl: paymentData.data.ttl, subscription: paymentData.data.subscription, errorMessage: $_('PROFILE_PIN_ERROR'), type: 'purchase' }, previousDialog: data.previousDialog });
			break;
		case 'onDialogPaymentFailed':
			data = event.getData();
			loadTemplate.call(this, { type: 'dialog', id: 'payment-done', conf: { key: data.key, title: $_('PAYMENT'), message: $_('PAYMENT_FAILED') + ' - ' + data.code, status: 'failed', code: 0 }});
			break;
		case 'onDialogCancelled':
		case 'onDialogDone':
			data = event.getData();
			if (data.previousDialog) {
				if (!data.previousDialog.id)
					data.previousDialog = {
						id: data.previousDialog
					};
				...
				if (data.previousDialog.id){
					switch (data.previousDialog.id) {
						...
						case 'payment-info':
							if (data.success)
								loadTemplate.call(this, { type: 'dialog', id: 'payment-done', result: data.result, conf: { key: data.key, title: $_('PAYMENT'), message: $_('PAYMENT_DONE'), status: 'ok', code: 0, result: data.result }});
							break;
					}
				}
				loadTemplate.call(this, data.previousDialog);
				return false;
			} else if (data.type && (data.type === 'purchase') && data.success) {
				loadTemplate.call(this, { type: 'dialog', id: 'payment-done', result: data.result, conf: { key: data.key, title: $_('PAYMENT'), message: $_('PAYMENT_DONE'), status: 'ok', code: 0, result: data.result }});
			}
			break;
