...
f(Player.state.INFOLOADED))},VideoPlayer.onPlaybackEnd=function(){d&&e&&(screen.log("EOF"),f(Player.state.EOF))},VideoPlayer.onPlaybackUnexpectedStop=function(){d&&e&&h&&(screen.log("STOPPED"),f(Player.state.STOP))},VideoPlayer.onPlaybackError=function(){d&&e&&(screen.log("ERROR"),f(Player.state.ERROR))});TVContext&&(TVContext.onContentSelectionSucceeded=function(){d||(screen.log("CHANNEL CHANGE"),fire.call(a,"onChannelChange"))});getter(a,"id",function(){return Player.TV});getter(a,"type",function(){return Player.type.VIDEO});
getter(a,"supports",function(){return k});getter(a,"waitIndicator",function(){return!1});getter(a,"channel",function(){var b=TVContext&&TVContext.getCurrentChannel()||{};return new TVChannel(b.number,b.name)});setter(a,"channel",function(b){TVContext&&TVContext.selectContentByID("service:number:"+b)});getter(a,"program",function(){var b=TVContext&&TVContext.getCurrentChannel(),b=b&&b.getCurrentProgram()||{};return new TVProgram(b.title,b.description,b.startTime,b.duration)});getter(a,"startTime",
...
getter(a,"buffered",function(){return 100});getter(a,"muted",function(){try{return AudioPlayer&&!0===AudioPlayer.muted||!1}catch(b){return!1}});setter(a,"muted",function(b){if(AudioPlayer)try{AudioPlayer.mute(!0===b)}catch(a){}});getter(a,"volume",function(){if(AudioPlayer)try{return AudioPlayer.level/100}catch(b){return 1}else return 1});setter(a,"volume",function(b){if(AudioPlayer)try{AudioPlayer.setVolume(parseInt(100*b,10))}catch(a){}});getter(a,"src",function(){return d&&e});setter(a,"src",function(b){if(b&&
VideoPlayer&&!e){h=!1;n=null;g=!1;c&&(clearTimeout(c),c=null);screen.log("BUFFERING");f(Player.state.BUFFERING);if(!d)try{VideoPlayer.grab(),d=!0}catch(a){return}else if(e&&VideoPlayer.status!==VideoPlayer.PLAYER_STATUS_STOPPED)try{VideoPlayer.stop()}catch(m){}try{screen.log("LOAD"),VideoPlayer.setURI(b),e=b,p.delay(800)}catch(k){screen.log("LOAD ERROR")}}else if(e&&VideoPlayer){h=!1;n=e=null;g=!1;c&&(clearTimeout(c),c=null);if(VideoPlayer.status!==VideoPlayer.PLAYER_STATUS_STOPPED)try{VideoPlayer.stop()}catch(l){}screen.log("STOP");
f(Player.state.STOP);(function(){if(d&&!e){d=!1;try{VideoPlayer.ungrab()}catch(b){}}}).delay(800)}});getter(a,"paused",function(){return g});setter(a,"paused",function(b){if(d&&VideoPlayer)if(!1===b&&!0===g)try{VideoPlayer.status!==VideoPlayer.PLAYER_STATUS_STOPPED?VideoPlayer.resume():VideoPlayer.start(),g=!1}catch(a){}else if(!0===b&&!1===g)try{VideoPlayer.pause(),g=!0}catch(n){}screen.log(g?"PAUSED":"RESUME");f(g?Player.state.PAUSE:Player.state.PLAY)});getter(a,"bounds",function(){return VideoPlayer?
[VideoPlayer.x,VideoPlayer.y,VideoPlayer.width,VideoPlayer.height]:l});setter(a,"bounds",function(b){var a=b&&b.length||0;1<a&&VideoPlayer&&(4===a&&VideoPlayer.setSize(b[2],b[3]),VideoPlayer.move(b[0],b[1]))})};NDSPlayer.prototype=new Player;NDSPlayer.prototype.constructor=NDSPlayer;plugins.players.push(new NDSPlayer);
