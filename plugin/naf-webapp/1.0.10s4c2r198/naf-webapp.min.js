...
NAF.Map=function(init,mediawareip){var RETRY_INTERVAL=1E4;var log=init.log;var mwip=mediawareip;var rid=1;var jsonVersion="2.0";var source;var that=this;var requestTimeout=init.requestTimeout?init.requestTimeout*1E3:0;var maxRetry=3;var retryCounter=0;var USER_AGE_RATING_OFF=1001;this.FACTORY_RESET_MODE={SIMPLE_RESET:0,ERASE_NAND_AND_RESET_LOCAL_SETTINGS:1,ERASE_NAND_AND_HDD_AND_RESET_LOCAL_SETTINGS:2,ERASE_NAND_AND_RESET_OPERATOR_DEFAULTS:3,ERASE_NAND_AND_HDD_AND_RESET_OPERATOR_DEFAULTS:4};var MapPlayerWindow=
function(){this.type=null;this.x=null;this.y=null;this.width=null;this.height=null};var MapPlayerParameters=function(){this.callbackParams=null;this.ver=null;this.url=null;this.block=null;this.rate=null;this.playPos=null;this.window=new MapPlayerWindow};var VidPreTuneParameters=function(){this.windowId=null;this.LocatorArray=null};var VidEnableSubtitleParameters=function(){this.enable=null};mapEmptyObject={};mapPlayerParams=new MapPlayerParameters;vidPreTuneParams=new VidPreTuneParameters;vidEnableSubtitleParams=
new VidEnableSubtitleParameters;vidBatchCommandParamsWithCmd={"commands":["GetAvailableAudioLanguages","GetAudioLanguage","GetAvailableSubtitleLanguages","GetSubtitleLanguage","IsTeletextAvailable"]};vidBatchCommandParamsWithoutCmd=["GetAvailableAudioLanguages","GetAudioLanguage","GetAvailableSubtitleLanguages","GetSubtitleLanguage","IsTeletextAvailable"];function internalHandler(event,message,error){var obj;try{obj=JSON.parse(event.data)}catch(e){log.error("server-sent event parsing failed, "+"event handler not invoked."+
e);error("Invalid server-sent event");return}if(isValidMWEvent(obj))message(obj);else{log.error("server-sent event is not a valid MW event, "+"event handler not invoked.");error("Invalid server-sent event")}function isValidMWEvent(obj){return typeof obj.method!=="undefined"}}this.SetAsyncEventHandlers=function(open,message,error){source=new EventSource("/server-sent-events");source.onopen=open;source.onmessage=function(evt){internalHandler(evt,message,error)};source.onerror=function(evt){var msg;
switch(evt.target.readyState){case EventSource.CONNECTING:msg="Eventsource reconnecting.";break;case EventSource.CLOSED:msg="Eventsource closed. Creating a new one.";setTimeout(function(){that.SetAsyncEventHandlers(open,message,error)},RETRY_INTERVAL);break}error(msg)}};function makeRequest(){var ajax=new XMLHttpRequest;var reqObj=new Object;reqObj.method=arguments[0];reqObj.id=rid++;reqObj.jsonrpc=jsonVersion;var params=new Object;for(var i=1;i<arguments.length;i+=2){var paramName=arguments[i];var paramValue=
arguments[i+1];params[paramName]=paramValue}reqObj.params=params;var request=JSON.stringify(reqObj);var reqUrl="/ajax?"+encodeURI(request);if(typeof mwip!="undefined"&&mwip!="")reqUrl="http://"+mwip+reqUrl;log.debug("MAP-REQ: "+request);ajax.open("GET",reqUrl,false);ajax.setRequestHeader("Content-Type","application/json;charset=UTF-8");ajax.send(null);var response;try{if(ajax.status==200){retryCounter=0;response=JSON.parse(ajax.responseText)}else{log.debug("MAP-RESPONSE: ERROR "+ajax.responseText);
if(ajax.status==500)if(retryCounter<maxRetry){retryCounter++;log.debug("MAP-RESPONSE: Retrying map request ...");makeRequest.apply(this,arguments);ajax.onreadystatechange=null;ajax.responseText=null;ajax=null;return}retryCounter=0;response={error:ajax.responseText,result:-1}}}catch(e){response={error:e,result:-1}}if(response.error)log.error("MAP-RESPONSE: ERROR code="+response.error.code+" message="+response.error.message);else if(log.getCurrLevel()<=4)log.debug("MAP-RESPONSE: ERROR status="+ajax.status+
" response="+ajax.responseText.substr(0,80));else log.debug("MAP-RESPONSE: ERROR status="+ajax.status+" response="+ajax.responseText);return response.result}function asyncMakeRequest(method,params,callback){var callbackParams=params.callbackParams;delete params.callbackParams;var xhttp=new XMLHttpRequest;var reqObj=new Object;reqObj.method=method;reqObj.id=rid++;reqObj.jsonrpc=jsonVersion;if(params&&JSON.stringify(params)!="{}")reqObj.params=params;var request=JSON.stringify(reqObj);var reqUrl="/ajax?"+
encodeURI(request);if(typeof mwip!="undefined"&&mwip!="")reqUrl="http://"+mwip+reqUrl;log.debug("MAP-ASYNC-REQ: "+request);xhttp.open("GET",reqUrl,true);xhttp.setRequestHeader("Content-Type","application/json;charset=UTF-8");xhttp.onreadystatechange=function(){if(xhttp.readyState===4){if(requestTimeout>0&&xhttpTimeout==null)return;if(xhttpTimeout!=null)clearTimeout(xhttpTimeout);var response;try{if(xhttp.status==200){retryCounter=0;if(log.getCurrLevel()<=4)log.debug("MAP-ASYNC-RESPONSE: SUCCESS "+
xhttp.responseText.substr(0,200));else log.debug("MAP-ASYNC-RESPONSE: SUCCESS "+xhttp.responseText);response=JSON.parse(xhttp.responseText)}else{log.debug("MAP-ASYNC-RESPONSE: ERROR "+xhttp.responseText);if(xhttp.status==500)if(retryCounter<maxRetry){retryCounter++;log.debug("MAP-ASYNC-RESPONSE: Retrying map request ...");asyncMakeRequest(method,params,callback);xhttpTimeout=null;xhttp.onreadystatechange=null;xhttp.responseText=null;xhttp=null;return}retryCounter=0;response={error:xhttp.responseText,
result:-1}}}catch(e){response={error:e,result:-1}}if(response.error)log.error("MAP-ASYNC-RESPONSE: ERROR code="+response.error.code+" message="+response.error.message);if(callback){if(response.error)response={error:response.error.message,result:-1};callback(response.result,callbackParams)}xhttpTimeout=null;xhttp.onreadystatechange=null;response=null;callbackParams=null;xhttp=null}};xhttp.send(null);if(requestTimeout>0)var xhttpTimeout=setTimeout(function(){xhttpTimeout=null;xhttp.abort();log.debug("MAP-ASYNC-RESPONSE: ERROR status = timeout. This request has timed out.");
if(callback)callback(-1,callbackParams);xhttpTimeout=null;xhttp.onreadystatechange=null;response=null;callbackParams=null;xhttp=null},requestTimeout);reqObj=null;request=null;reqUrl=null}this.MapSetSplashEnabled=function(isSplash,callback,callbackParams){asyncMakeRequest("MapSetSplashEnabled",{enable:isSplash,callbackParams:callbackParams},callback)};this.VidSetAnimationParameters=function(win,bAnim,dur,speeed,fps,callback){asyncMakeRequest("VidSetAnimationParameters",{"windowId":win,"bAnimate":bAnim,
"duration":dur,"speed":speed,"fps":fps},callback)};this.VidPretune=function(win,aLocators,callback){vidPreTuneParams.windowId=win;vidPreTuneParams.LocatorArray=aLocators;asyncMakeRequest("VidPreTune",vidPreTuneParams,callback)};this.VidStopPretune=function(win,aLocators,callback){asyncMakeRequest("VidStopPreTune",{"windowId":win},callback)};this.VidGetCurChan=function(callback){asyncMakeRequest("VidGetCurChan",mapEmptyObject,callback)};this.VidGetLastChan=function(callback){asyncMakeRequest("VidGetLastChan",
mapEmptyObject,callback)};this.VidSetLastViewedChan=function(chanId,callback){asyncMakeRequest("MapSetLastChan",{"id":chanId},callback)};this.VidGetStartChan=function(){return null};this.VidDetune=function(callback){asyncMakeRequest("VidDetune",mapEmptyObject,callback)};this.VidGetChanCount=function(callback){asyncMakeRequest("VidGetChanCount",mapEmptyObject,callback)};this.VidGetChannelList=function(platform,callback){asyncMakeRequest("VidGetChannelList",{"platform":platform},callback)};this.VidSetVideoWinSize=
function(x,y,w,h,bPIP,callback){asyncMakeRequest("VidSetVideoWinSize",{"x":x,"y":y,"w":w,"h":h,"bPIP":bPIP},callback)};this.VidGetStreamComponents=function(callback){if(callback)asyncMakeRequest("VidBatchCommand",vidBatchCommandParamsWithCmd,callback);else return makeRequest("VidBatchCommand","commands",vidBatchCommandParamsWithoutCmd)};this.VidEnableCC=function(bEnable,callback){asyncMakeRequest("VidSetCloseCap",{"enable":bEnable},callback)};this.VidGetAvailableAudioLang=function(callback){asyncMakeRequest("GetAvailableAudioLanguages",
mapEmptyObject,callback)};this.VidSetPrefAudioLang=function(lang,type,callback,callbackParams){var params=new Object;params["language"]=lang;if(type||type!="")params["type"]=type;params.callbackParams=callbackParams;asyncMakeRequest("SetAudioLanguagePreference",params,callback)};this.VidGetPrefAudioLang=function(callback){asyncMakeRequest("GetAudioLanguagePreference",mapEmptyObject,callback)};this.VidSetAudioLang=function(lang,callback,callbackParams){var params=new Object;params["language"]=lang;
params.callbackParams=callbackParams;asyncMakeRequest("SetAudioLanguage",params,callback)};this.VidGetAudioLang=function(callback){asyncMakeRequest("GetAudioLanguage",mapEmptyObject,callback)};this.VidSetResolution=function(port,res,callback){asyncMakeRequest("VidSetResolution",{"port":port,"resolution":res},callback)};this.VidGetAvailableSubtitleLang=function(callback){asyncMakeRequest("GetAvailableSubtitleLanguages",mapEmptyObject,callback)};this.VidSetPrefSubtitleLang=function(langs,type,callback,
callbackParams){var params=new Object;params["language"]=langs;params["type"]=type;params.callbackParams=callbackParams;asyncMakeRequest("SetSubtitleLanguagePreference",params,callback)};this.VidGetPrefSubtitleLang=function(callback){asyncMakeRequest("GetSubtitleLanguagePreference",mapEmptyObject,callback)};this.VidGetSubtitleAvailability=function(callback){asyncMakeRequest("GetSubtitleAvailability",mapEmptyObject,callback)};this.VidEnableSubtitle=function(bo,callback){vidEnableSubtitleParams.enable=
bo;asyncMakeRequest("EnableSubtitle",vidEnableSubtitleParams,callback)};this.VidSetSubtitleLang=function(langs,callback,callbackParams){var params=new Object;params["language"]=langs;params.callbackParams=callbackParams;asyncMakeRequest("SetSubtitleLanguage",params,callback)};this.VidGetSubtitleLang=function(callback){asyncMakeRequest("GetSubtitleLanguage",mapEmptyObject,callback)};this.IsSubtitleEnabled=function(callback){asyncMakeRequest("IsSubtitleEnabled",mapEmptyObject,callback)};this.VidSetPrefTeletextLang=
function(lang,type,callback,callbackParams){log.warn("VidSetPrefTeletextLang(): Not yet supported.");return};this.VidGetPrefTeletextLang=function(callback){log.warn("VidGetPrefTeletextLang(): Not yet supported.");return};this.IsTeletextEnabled=function(callback){asyncMakeRequest("IsTeletextEnabled",mapEmptyObject,callback)};this.EnableTeletext=function(enable,callback){asyncMakeRequest("EnableTeletext",{"enable":enable},callback)};this.IsTeletextAvailable=function(callback){asyncMakeRequest("IsTeletextAvailable",
mapEmptyObject,callback)};this.VidSetTeletextLang=function(lang,type,callback,callbackParams){log.warn("VidSetTeletextLang(): Not yet supported.");return};this.VidGetTeletextLang=function(callback){log.warn("VidGetTeletextLang(): Not yet supported.");return};this.VidSpeakerMute=function(isBlocked,callback){asyncMakeRequest("VidSpeakerMute",{"flag":isBlocked},callback)};this.VidDisplayVideoOn=function(isBlocked,callback){asyncMakeRequest("VidDisplayVideoOn",{"flag":isBlocked},callback)};this.DlnaSetDeviceName=
function(name,type,callback){if(callback)asyncMakeRequest("MapDLNASetDeviceName",{"name":name,"type":type},callback);else return makeRequest("MapDLNASetDeviceName","name",name,"type",type)};this.DlnaGetDeviceName=function(type,callback){if(callback)asyncMakeRequest("MapDLNAGetDeviceName",{"type":type},callback);else return makeRequest("MapDLNAGetDeviceName","type",type)};this.MapDLNAGetActiveDevices=function(params,callback){if(callback)asyncMakeRequest("MapDLNAGetActiveDevices",{"type":params},callback);
else return makeRequest("MapDLNAGetActiveDevices","type",params)};this.MapDLNAGetContainer=function(devid,containerId,containerType,filter,start,count,sort,callback){if(callback)asyncMakeRequest("MapDLNAGetContainer",{"id":devid,"containerId":containerId,"containerType":containerType,"filter":filter,"startAt":start,"requestedCount":count,"sort":sort},callback);else return makeRequest("MapDLNAGetContainer","id",devid,"containerId",containerId,"containerType",containerType,"filter",filter,"startAt",
start,"requestedCount",count,"sort",sort)};this.MapPlayer=function(url,block,rate,position,winType,x,y,w,h,callback,callbackParams){if(callbackParams)mapPlayerParams.callbackParams=callbackParams;else mapPlayerParams.callbackParams=undefined;mapPlayerParams.ver="1.0";if(url)mapPlayerParams.url=url;else mapPlayerParams.url=undefined;mapPlayerParams.block=block;mapPlayerParams.rate=rate;if(position)mapPlayerParams.playPos=position;else mapPlayerParams.playPos=undefined;mapPlayerParams.window.type=winType;
mapPlayerParams.window.x=x;mapPlayerParams.window.y=y;mapPlayerParams.window.width=w;mapPlayerParams.window.height=h;asyncMakeRequest("MapPlayer",mapPlayerParams,callback)};this.MapPlayerRTSPVod=function(assetId,block,rate,position,winType,x,y,w,h,callback,callbackParams){var playerWindow=new Object;playerWindow.type=winType;playerWindow.x=x;playerWindow.y=y;playerWindow.width=w;playerWindow.height=h;var params=new Object;params["ver"]="1.0";if(assetId)params["id"]=assetId;params["block"]=block;params["rate"]=
rate;if(position!=undefined&&position!=null&&position!=="")if(position>=0)params["playPos"]=position*1E3;else if(position==-1)params["playPos"]=-1;params["window"]=playerWindow;if(callbackParams)params.callbackParams=callbackParams;asyncMakeRequest("MapPlayer",params,callback);playerWindow=null;params=null};this.MapPlayerStop=function(winType,x,y,w,h,callback){var playerWindow=new Object;playerWindow.type=winType;asyncMakeRequest("MapPlayer",{"ver":"1.0","window":playerWindow},callback);playerWindow=
null};this.MapPlayerSkip=function(url,rate,offset,winType,x,y,w,h,callback,callbackParams,block){var playerWindow=new Object;playerWindow.type=winType;playerWindow.x=x;playerWindow.y=y;playerWindow.width=w;playerWindow.height=h;var params={"ver":"1.0","url":url,"rate":rate,"skip":offset,"window":playerWindow};if(callbackParams)params.callbackParams=callbackParams;if(block!==undefined&&block!==null)params.block=block;asyncMakeRequest("MapPlayer",params,callback);playerWindow=null};this.MapPlayerMakePipPrimary=
function(pipUrl,callback){var params=new Object;params["ver"]="1.0";params["url"]=pipUrl;params["window"]={type:"pip"};params["destwin"]={type:"main"};asyncMakeRequest("MapPlayer",params,callback)};this.MapPlayerSetVideoWinSize=function(url,winType,x,y,w,h,callback){var playerWindow=new Object;playerWindow.type=winType;playerWindow.x=x;playerWindow.y=y;playerWindow.width=w;playerWindow.height=h;asyncMakeRequest("MapPlayer",{"ver":"1.0","url":url,"window":playerWindow},callback)};this.DvrGetCheckPoint=
function(callback){if(callback)asyncMakeRequest("PvrGetCheckPoint",mapEmptyObject,callback);else return makeRequest("PvrGetCheckPoint")};this.DvrSync=function(callback){if(callback)asyncMakeRequest("PvrSync",mapEmptyObject,callback);else return makeRequest("PvrSync")};this.DvrRecord=function(loc,channelId,secs,t,d,start,repeat,fPad,bPad,locked,callback){var params=new Object;params["locator"]=loc;params["channelId"]=channelId;params["duration"]=secs;params["title"]=t;params["description"]=d;params["start"]=
start;params["isPinLocked"]=locked;if(fPad)params["frontPad"]=fPad;if(bPad)params["backPad"]=bPad;var crontabInterval;if(repeat!=0){var recordingDate;if(!start)recordingDate=new Date;else recordingDate=new Date(start*1E3);var repeatDays="";for(var i=0;i<repeat.length;i++)if(repeat[i]==7)repeatDays="0,"+repeatDays;else repeatDays+=repeat[i]+",";repeatDays=repeatDays.substring(0,repeatDays.length-1);crontabInterval=recordingDate.getMinutes()+" "+recordingDate.getUTCHours()+" * * "+repeatDays}if(crontabInterval)params["interval"]=
crontabInterval;asyncMakeRequest("PvrSchedRecording",params,callback);params=null};this.DvrRecordEvent=function(eId,fPad,bPad,locked,callback){log.trace("DvrRecordEvent: "+eId);var params=new Object;params["eventCrid"]=eId;params["isPinLocked"]=locked;if(fPad||fPad===0)params["frontPad"]=fPad;if(bPad||bPad===0)params["backPad"]=bPad;asyncMakeRequest("PvrSchedRecording",params,callback);params=null};this.DvrRecordSeries=function(sId,fPad,bPad,chan,event,locked,callback){log.trace("DvrRecordSeries: "+
sId+" chan: "+chan+" event: "+event);var params=new Object;params["seriesCrid"]=sId;params["isPinLocked"]=locked;if(fPad!=null)params["frontPad"]=fPad;if(bPad!=null)params["backPad"]=bPad;if(chan)params["channel"]=chan;if(event)params["event"]=event;asyncMakeRequest("PvrSchedRecording",params,callback);params=null};this.DvrUpdateRecord=function(id,duration,startTime,callback){if(duration!=undefined&&startTime!=undefined)if(callback)asyncMakeRequest("PvrUpdateRecording",{"recordingId":id,"duration":duration,
"start":startTime},callback);else return makeRequest("PvrUpdateRecording","recordingId",id,"duration",duration,"start",startTime);else if(startTime!=undefined){log.warn("DvrUpdateRecord(): duration is null!!!");if(callback)asyncMakeRequest("PvrUpdateRecording",{"recordingId":id,"start":startTime},callback);else return makeRequest("PvrUpdateRecording","recordingId",id,"start",startTime)}else if(duration!=undefined){log.warn("DvrUpdateRecord(): startTime is null!!!");if(callback)asyncMakeRequest("PvrUpdateRecording",
{"recordingId":id,"duration":duration},callback);else return makeRequest("PvrUpdateRecording","recordingId",id,"duration",duration)}};this.DvrUpdateOperation=function(id,fPad,bPad,locked,callback){var params=new Object;params["operationId"]=id;if(fPad)params["frontPad"]=fPad;if(bPad)params["backPad"]=bPad;if(locked!=undefined)params["isPinLocked"]=locked;asyncMakeRequest("MapPvrUpdateOperation",params,callback);params=null};this.DvrSetBookmark=function(recordingId,bookmark,callback){asyncMakeRequest("PvrSetBookmark",
{"recordingId":recordingId,"position":bookmark},callback)};this.DvrSetViewState=function(recordingId,viewState,callback){asyncMakeRequest("PvrSetViewState",{"recordingId":recordingId,"ViewState":viewState},callback)};this.DvrAssetList=function(callback){asyncMakeRequest("PvrListRecording",mapEmptyObject,callback)};this.DvrListRequests=function(callback){if(callback)asyncMakeRequest("PvrListRequests",mapEmptyObject,callback);else return makeRequest("PvrListRequests")};this.DvrListOperations=function(callback){if(callback)asyncMakeRequest("PvrListOperations",
mapEmptyObject,callback);else return makeRequest("PvrListOperations")};this.DvrListRecordings=function(callback){if(callback)asyncMakeRequest("PvrListRecordings",mapEmptyObject,callback);else return makeRequest("PvrListRecordings")};this.DvrListRequestsOperationsRecordings=function(callback){if(callback)asyncMakeRequest("PvrBatchCommand",{"commands":["PvrListRequests","PvrListOperations","PvrListRecordings"]},callback);else return makeRequest("PvrBatchCommand","commands",["PvrListRequests","PvrListOperations",
"PvrListRecordings"])};this.DvrGetRecords=function(cp,callback){if(callback)asyncMakeRequest("MapPvrGetRecords",{"checkPoint":cp},callback);else return makeRequest("MapPvrGetRecords","checkPoint",cp)};this.DvrGetRecordingInfo=function(id,callback){if(callback)asyncMakeRequest("PvrGetRecordingInfo",{"recordingId":id},callback);else return makeRequest("PvrGetRecordingInfo","recordingId",id)};this.DvrGetConflicts=function(id,callback,callbackParams){var params=new Object;params={"opId":id};params.callbackParams=
callbackParams;asyncMakeRequest("MapPvrGetConflicts",params,callback)};this.DvrResolve=function(id,callback){asyncMakeRequest("MapPvrResolve",{"operationId":id},callback)};this.DvrPause=function(callback){asyncMakeRequest("PvrPlayTrick",{"speed":0},callback)};this.DvrResume=function(callback){asyncMakeRequest("PvrPlayTrick",{"speed":100},callback)};this.DvrFastForward=function(speed,callback){asyncMakeRequest("PvrPlayTrick",{"speed":speed},callback)};this.DvrRewind=function(speed,callback){asyncMakeRequest("PvrPlayTrick",
{"speed":-speed},callback)};this.DvrOffset=function(offset,callback){asyncMakeRequest("PvrPlaySkip",{"offset":offset},callback)};this.DvrDelete=function(loc,callback){asyncMakeRequest("PvrDeleteRecording",{"locator":loc},callback)};this.DvrDeleteAll=function(callback){asyncMakeRequest("PvrDeleteAll",mapEmptyObject,callback)};this.DvrDeleteRequest=function(id,callback){asyncMakeRequest("PvrDelete",{"requestId":id},callback)};this.DvrDeleteOperation=function(id,callback){asyncMakeRequest("PvrDelete",
{"operationId":id},callback)};this.DvrDeleteOperations=function(ids,callback){var command={};command["operationIds"]=ids;asyncMakeRequest("PvrDelete",command,callback)};this.DvrDeleteRecording=function(id,callback){asyncMakeRequest("PvrDelete",{"recordingId":id},callback)};this.DvrCancelRequest=function(id,callback){asyncMakeRequest("PvrCancel",{"requestId":id},callback)};this.DvrDeleteProtectOperation=function(id,state,callback,callbackParams){var params=new Object;params={"operationId":id,"protect":state};
params.callbackParams=callbackParams;asyncMakeRequest("PvrDeleteProtect",params,callback)};this.DvrDeleteProtectOperations=function(ids,state,callback,callbackParams){var params=new Object;params={"operationIds":ids,"protect":state};params.callbackParams=callbackParams;asyncMakeRequest("PvrDeleteProtect",params,callback)};this.DvrDeleteProtectRequest=function(id,state,callback,callbackParams){var params=new Object;params={"requestId":id,"protect":state};params.callbackParams=callbackParams;asyncMakeRequest("PvrDeleteProtect",
params,callback)};this.DvrPinProtectOperation=function(id,state,callback,callbackParams){var params=new Object;params={"operationId":id,"protect":state};params.callbackParams=callbackParams;asyncMakeRequest("PvrPinProtect",params,callback)};this.DvrPinProtectOperations=function(ids,state,callback,callbackParams){var params=new Object;params={"operationIds":ids,"protect":state};params.callbackParams=callbackParams;asyncMakeRequest("PvrPinProtect",params,callback)};this.DvrPinProtectRequest=function(id,
state,callback,callbackParams){var params=new Object;params={"requestId":id,"protect":state};params.callbackParams=callbackParams;asyncMakeRequest("PvrPinProtect",params,callback)};this.DvrStop=function(callback){asyncMakeRequest("DvrStop",mapEmptyObject,callback)};this.PvrStop=function(callback){asyncMakeRequest("PvrStop",mapEmptyObject,callback)};this.DvrStopRecording=function(id,callback){asyncMakeRequest("PvrStopRecording",{"recordingId":id},callback)};this.VodSessionOpen=function(aid,typ,callback){asyncMakeRequest("VodSessionOpen",
{"assetId":aid,"vodType":typ},callback)};this.VodSessionClose=function(callback){asyncMakeRequest("VodSessionClose",mapEmptyObject,callback)};this.VodPlay=function(blocked,start,callback){asyncMakeRequest("VodPlay",{"blocked":blocked,"start":start},callback)};this.VodPause=function(callback){asyncMakeRequest("VodPause",mapEmptyObject,callback)};this.VodFastforward=function(num,denom,callback){asyncMakeRequest("VodFastforward",{"numerator":num,"denominator":denom},callback)};this.VodRewind=function(num,
denom,callback){asyncMakeRequest("VodRewind",{"numerator":num,"denominator":denom},callback)};this.VodStop=function(callback){asyncMakeRequest("VodStop",mapEmptyObject,callback)};this.VodGet=function(nam,callback){asyncMakeRequest("VodGet",{"name":nam},callback)};this.VodSet=function(nam,val,callback){asyncMakeRequest("VodSet",{"name":nam,"value":val},callback)};this.VodPlaySkip=function(offset,callback){asyncMakeRequest("VodPlaySkip",{"offset":offset},callback)};this.VodResume=function(callback){asyncMakeRequest("VodFastforward",
{"numerator":1,"denominator":1},callback)};this.ConfigGet=function(nm,callback,callbackParams){var params=new Object;params={"name":nm};if(callbackParams)params.callbackParams=callbackParams;asyncMakeRequest("ConfigGet",params,callback)};this.ConfigsGet=function(nmArray,callback,callbackParams){var params=new Object;params={"name":nmArray};if(callbackParams)params.callbackParams=callbackParams;asyncMakeRequest("ConfigGet",params,callback)};this.ConfigSet=function(nm,val,callback){if(callback)asyncMakeRequest("ConfigSet",
{"name":nm,"value":val},callback);else return makeRequest("ConfigSet","name",nm,"value",val)};this.GetMWConfig=function(section,key,callback){asyncMakeRequest("GetMWConfig",{"section":section,"key":key},callback)};this.GetKeyMap=function(callback){asyncMakeRequest("GetKeyMap",mapEmptyObject,callback)};this.EmulateRemoteKey=function(kc,ks){if(!!ks)asyncMakeRequest("EmulateRemoteKey",{"KeyCode":kc,"KeyState":ks});else asyncMakeRequest("EmulateRemoteKey",{"KeyCode":kc})};this.VerifyPin=function(profId,
type,pin,callback,callbackParams){var params=new Object;params={"profile":profId,"type":type,"pin":pin};params.callbackParams=callbackParams;asyncMakeRequest("VerifyPin",params,callback)};this.ResetPin=function(profId,type,pin,newpin,callback,callbackParams){var params=new Object;params={"profile":profId,"type":type,"masterpin":pin,"newpin":newpin};params.callbackParams=callbackParams;asyncMakeRequest("ResetPin",params,callback)};this.GetPinState=function(profId,type){asyncMakeRequest("GetPinState",
{"profile":profId,"type":type},callback)};this.ActivatePin=function(profId,type,pin,enable){asyncMakeRequest("ActivatePin",{"profile":profId,"type":type,"masterpin":masterpin,"activate":enable},callback)};this.GetProfileNames=function(callback,callbackParams){callback()};this.GetProfileSettings=function(profileid,settings,callback,callbackParams){var params=new Object;if(callbackParams)params.callbackParams=callbackParams;params.profileId=profileid;params.Settings=settings;asyncMakeRequest("ReadProfile",
params,callback)};this.SetProfileSettings=function(profileid,settings,callback,callbackParams){var params=new Object;if(callbackParams)params.callbackParams=callbackParams;params.profileId=profileid;if(profileid.indexOf("MasterProfile")!=-1)params.keyword="master";params.Settings=settings;asyncMakeRequest("WriteProfile",params,callback)};this.ProfileSetInitChan=function(profileid,newInitChan,callback,callbackParams){var params=new Object;if(callbackParams)params.callbackParams=callbackParams;params.profileId=
profileid;if(profileid.indexOf("MasterProfile")!=-1)params.keyword="master";params.Settings=new Object;params.Settings.initChannel=newInitChan;asyncMakeRequest("WriteProfile",params,callback)};this.ProfileGetInitChan=function(profileid,callback,callbackParams){var params=new Object;if(callbackParams)params.callbackParams=callbackParams;params.profileId=profileid;params.Settings=["initChannel"];asyncMakeRequest("ReadProfile",params,callback)};this.ProfileSetAgeRating=function(profileid,newAgeRating,
...
callbackParams;params.profileId=profileid;params.Settings=["ageRating"];asyncMakeRequest("ReadProfile",params,callback)};this.ProfileSetStartupProtection=function(profileid,newStartupProtection,callback,callbackParams){var params=new Object;if(callbackParams)params.callbackParams=callbackParams;params.profileId=profileid;if(profileid.indexOf("MasterProfile")!=-1)params.keyword="master";params.Settings=new Object;params.Settings.startupProtection=newStartupProtection;asyncMakeRequest("WriteProfile",
params,callback)};this.ProfileGetStartupProtection=function(profileid,callback,callbackParams){var params=new Object;if(callbackParams)params.callbackParams=callbackParams;params.profileId=profileid;params.Settings=["startupProtection"];asyncMakeRequest("ReadProfile",params,callback)};this.ProfileSetNeedPinForPurchase=function(profileid,newNeedPinForPurchase,callback,callbackParams){var params=new Object;if(callbackParams)params.callbackParams=callbackParams;params.profileId=profileid;if(profileid.indexOf("MasterProfile")!=
...
{"mode":mode,"timeout":timeout,"time":time,"callbackParams":callbackParams},callback)};this.AcknowledgePowerManagerEvent=function(callback){asyncMakeRequest("AcknowledgePowerManagerEvent",mapEmptyObject,callback)};this.PesGetAllEvents=function(filter,callback){if(callback)asyncMakeRequest("GetAllEvents",{"filter":filter},callback);else return makeRequest("PesGetAllEvents","filter",filter)};this.PesGetChannelsWithEvent=function(chanFilter,chanProps,eventFilter,callback,callbackParams){asyncMakeRequest("GetChannelsWithEvent",
{"ChanFilter":chanFilter,"ChanProps":chanProps,"EventFilter":eventFilter,"callbackParams":callbackParams},callback)};this.PesGetEventsByChanId=function(chanId,filter,callback){if(callback)asyncMakeRequest("GetEventsByChanId",{"chanId":chanId,"filter":filter},callback);else return makeRequest("GetEventsByChanId","chanId",chanId,"filter",filter)};this.PesGetEventsByLcn=function(lcn,filter,callback){if(callback)asyncMakeRequest("GetEventsByLcn",{"lcn":lcn,"filter":filter},callback);else return makeRequest("GetEventsByLcn",
"lcn",lcn,"filter",filter)};this.PesGetPfEventsByChanId=function(chanId){return makeRequest("GetPfEventsByChanId","chanId",chanId)};this.PesGetPfEventsByLcn=function(lcn){return makeRequest("GetPfEventsByLcn","lcn",lcn)};this.MapGetOnlineMode=function(callback,checkNow){if(callback){var params=checkNow?{"checkNow":true}:{};asyncMakeRequest("MapGetOnlineMode",params,callback)}else return makeRequest("MapGetOnlineMode")};this.writePVRProfile=function(profileid,pvrSettings,callback,callbackParams){var params=
new Object;params.callbackParams=callbackParams;params.profileId=profileid;if(profileid.indexOf("MasterProfile")!=-1)params.keyword="master";params.Settings=new Object;params.Settings.pvrSettings=new Object;params.Settings.pvrSettings.frontPad=pvrSettings.newFrontPad!=undefined?pvrSettings.newFrontPad:pvrSettings.frontPad;params.Settings.pvrSettings.backPad=pvrSettings.newBackPad!=undefined?pvrSettings.newBackPad:pvrSettings.backPad;params.Settings.pvrSettings.seriesDomain=pvrSettings.newSeriesDomain!=
undefined?pvrSettings.newSeriesDomain:pvrSettings.seriesDomain;params.Settings.pvrSettings.diskManagement=pvrSettings.newDiskManagement!=undefined?pvrSettings.newDiskManagement:pvrSettings.diskManagement;asyncMakeRequest("WriteProfile",params,callback)};this.readPVRProfile=function(profileid,callback,callbackParams){var params=new Object;if(callbackParams)params.callbackParams=callbackParams;params.profileId=profileid;if(profileid.indexOf("MasterProfile")!=-1)params.keyword="master";params.Settings=
["pvrSettings"];asyncMakeRequest("ReadProfile",params,callback)};this.writeViewSettingsProfile=function(profileid,langSettings,viewSettings,callback,callbackParams){var params=new Object;params.callbackParams=callbackParams;params.profileId=profileid;if(profileid.indexOf("MasterProfile")!=-1)params.keyword="master";params.Settings=new Object;params.Settings.viewSettings=new Object;params.Settings.viewSettings.lang=new Object;params.Settings.viewSettings.lang.code=langSettings.newCode!=undefined?langSettings.newCode:
langSettings.code;params.Settings.viewSettings.lang.type=langSettings.newType!=undefined?langSettings.newType:langSettings.type;params.Settings.viewSettings.transparency=viewSettings.newTransparency!=undefined?viewSettings.newTransparency:viewSettings.transparency;params.Settings.viewSettings.bannerTimeout=viewSettings.newBannerTimeout!=undefined?viewSettings.newBannerTimeout:viewSettings.bannerTimeout;params.Settings.viewSettings.epgStyle=viewSettings.newEpgStyle!=undefined?viewSettings.newEpgStyle:
viewSettings.epgStyle;params.Settings.viewSettings.epgSpan=viewSettings.newEpgSpan!=undefined?viewSettings.newEpgSpan:viewSettings.epgSpan;params.Settings.viewSettings.assetsStyle=viewSettings.newAssetsStyle!=undefined?viewSettings.newAssetsStyle:viewSettings.assetsStyle;params.Settings.viewSettings.settingsProtection=viewSettings.newSettingsProtection!=undefined?viewSettings.newSettingsProtection:viewSettings.settingsProtection;asyncMakeRequest("WriteProfile",params,callback)};this.readViewSettingsProfile=
...
-1)params.keyword="master";params.Settings=new Object;params.Settings.outputSettings=new Object;params.Settings.outputSettings.primary=outputSettings.newPrimary!=undefined?outputSettings.newPrimary:outputSettings.primary;params.Settings.outputSettings.hdmi=new Object;params.Settings.outputSettings.hdmi.resolution=outputSettings.hdmi.newResolution!=undefined?outputSettings.hdmi.newResolution:outputSettings.hdmi.resolution;params.Settings.outputSettings.hdmi.format=outputSettings.hdmi.newFormat!=undefined?
outputSettings.hdmi.newFormat:outputSettings.hdmi.format;params.Settings.outputSettings.hdmi.color=outputSettings.hdmi.newColor!=undefined?outputSettings.hdmi.newColor:outputSettings.hdmi.color;params.Settings.outputSettings.hdmi.audio=new Object;params.Settings.outputSettings.hdmi.audio.enable=outputSettings.hdmi.audio.newEnable!=undefined?outputSettings.hdmi.audio.newEnable:outputSettings.hdmi.audio.enable;params.Settings.outputSettings.hdmi.audio.delay=outputSettings.hdmi.audio.newDelay!=undefined?
outputSettings.hdmi.audio.newDelay:outputSettings.hdmi.audio.delay;params.Settings.outputSettings.spdif=new Object;params.Settings.outputSettings.spdif.enable=outputSettings.spdif.newEnable!=undefined?outputSettings.spdif.newEnable:outputSettings.spdif.enable;params.Settings.outputSettings.spdif.delay=outputSettings.spdif.newDelay!=undefined?outputSettings.spdif.newDelay:outputSettings.spdif.delay;params.Settings.outputSettings.tv=new Object;params.Settings.outputSettings.tv.aspectRatio=outputSettings.tv.newAspectRatio!=
undefined?outputSettings.tv.newAspectRatio:outputSettings.tv.aspectRatio;params.Settings.outputSettings.tv.start=outputSettings.tv.newStart!=undefined?outputSettings.tv.newStart:outputSettings.tv.start;params.Settings.outputSettings.scart=new Object;params.Settings.outputSettings.scart.format=outputSettings.scart.newFormat!=undefined?outputSettings.scart.newFormat:outputSettings.scart.format;params.Settings.outputSettings.scart.color=outputSettings.scart.newColor!=undefined?outputSettings.scart.newColor:
outputSettings.scart.color;params.Settings.outputSettings.scart.enable=outputSettings.scart.newEnable!=undefined?outputSettings.scart.newEnable:outputSettings.scart.enable;params.Settings.outputSettings.audio=new Object;params.Settings.outputSettings.audio.format=outputSettings.audio.newFormat!=undefined?outputSettings.audio.newFormat:outputSettings.audio.format;params.Settings.outputSettings.frontPanel=new Object;params.Settings.outputSettings.frontPanel.luminosity=outputSettings.frontPanel.newLuminosity!=
undefined?outputSettings.frontPanel.newLuminosity:outputSettings.frontPanel.luminosity;asyncMakeRequest("WriteProfile",params,callback)};this.readOutputSettingsProfile=function(profileid,callback,callbackParams){var params=new Object;if(callbackParams)params.callbackParams=callbackParams;params.profileId=profileid;if(profileid.indexOf("MasterProfile")!=-1)params.keyword="master";params.Settings=["outputSettings"];asyncMakeRequest("ReadProfile",params,callback)};this.SetVideoOutputSettings=function(resolution,
type,callback,callbackParams){resolution=NAF.conf.hdmiComponent[resolution];var params=new Object;params.id=resolution;if(type)params.type=type;params.callbackParams=callbackParams;asyncMakeRequest("MapSetVideoOutputPortSettings",params,callback)};this.GetVideoOutputSettings=function(resolution,callback){params=new Object;if(resolution)params.type=resolution;asyncMakeRequest("MapGetVideoOutputPortSettings",params,callback)};this.SetPortControlSettings=function(aspectRatio,fastBlank,callback,callbackParams){var params=
new Object;params.type="scart";if(aspectRatio)params.aspectRatio=aspectRatio;if(fastBlank)params.fastBlank=fastBlank;params.callbackParams=callbackParams;asyncMakeRequest("MapSetVideoPortControlSettings",params,callback)};this.MapGetDolbyEnabled=function(callback){asyncMakeRequest("MapGetDolbyEnabled",mapEmptyObject,callback)};this.MapSetDolbyEnabled=function(isDolby,callback,callbackParams){asyncMakeRequest("MapSetDolbyEnabled",{enable:isDolby,callbackParams:callbackParams},callback)};this.SetAudioOutputSettings=
function(format,type,callback,callbackParams){var params=new Object;params.id=format;if(type)params.type=type;params.callbackParams=callbackParams;asyncMakeRequest("MapSetAudioOutputPortSettings",params,callback)};this.GetAudioOutputSettings=function(format,callback){params=new Object;if(format)params.type=format;asyncMakeRequest("MapGetAudioOutputPortSettings",params,callback)};this.SetAudioControlSettings=function(params,callback,callbackParams){if(callbackParams)params.callbackParams=callbackParams;
asyncMakeRequest("MapSetAudioControlSettings",params,callback)};this.GetAudioControlSettings=function(type,callback,callbackParams){var params=new Object;if(callbackParams)params.callbackParams=callbackParams;if(type)params.type=type;asyncMakeRequest("MapGetAudioControlSettings",params,callback)};this.GetHDDAvailability=function(callback){asyncMakeRequest("MapGetHDDAvailability",mapEmptyObject,callback)};this.MapGetSystemStatus=function(callback){asyncMakeRequest("MapGetSystemStatus",mapEmptyObject,
callback)};this.VidGetTSBInfo=function(callback){asyncMakeRequest("GetTSBInfo",mapEmptyObject,callback)};this.VidGetTSBEventInfo=function(callback){asyncMakeRequest("GetTSBEventInfo",mapEmptyObject,callback)};this.SubscribePlayerPosition=function(frequency,callback){asyncMakeRequest("MAPSubscribeEvent",{"name":"MWPlayerPosition","frequency":frequency},callback)};this.UnsubscribePlayerPosition=function(callback){asyncMakeRequest("MAPUnsubscribeEvent",{"name":"MWPlayerPosition"},callback)};this.SubscribeTSBEventInfo=
function(callback){asyncMakeRequest("MAPSubscribeEvent",{"name":"TSBEventInfo"},callback)};this.UnsubscribeTSBEventInfo=function(callback){asyncMakeRequest("MAPUnsubscribeEvent",{"name":"TSBEventInfo"},callback)};this.AdjustUserAgeRating=function(ageRating,callback){asyncMakeRequest("AdjustUserAgeRating",{"tempAgeRating":ageRating},callback)};this.RevertUserAgeRating=function(callback){asyncMakeRequest("RevertUserAgeRating",mapEmptyObject,callback)};this.GetDiagnostics=function(callback){asyncMakeRequest("GetDiagnostics",
mapEmptyObject,function(result){if(callback)callback(GetDiagnosticsCB(result))})};this.DiagnosticsGetHDMIStatus=function(callback){asyncMakeRequest("MapGetVideoPortControlSettings",mapEmptyObject,callback)};this.SendDiagMsg=function(result,targetIp,msg,callback){if(callback)asyncMakeRequest("SendTo",{"method":"DiagResult","targetIP":targetIp,"result":result,"message":msg},callback);else return makeRequest("SendTo","method","DiagResult","targetIP",targetIp,"result",result,"message",msg)};this.SendDiagResp=
function(result,targetIp,msg,dqId,callback){if(callback)asyncMakeRequest("QueryResult",{"method":"DiagResult","targetIP":targetIp,"result":result,"message":msg,"dqId":dqId},callback);else return makeRequest("QueryResult","method","DiagResult","targetIP",targetIp,"result",result,"message",msg,"dqId",dqId)};this.SetDefaultStandbyMode=function(profileid,defaultStandby,callback,callbackParams){var params=new Object;if(callbackParams)params.callbackParams=callbackParams;params.profileId=profileid;if(profileid.indexOf("MasterProfile")!=
-1)params.keyword="master";params.Settings=new Object;params.Settings.defaultStandby=defaultStandby;asyncMakeRequest("WriteProfile",params,callback)};this.GetDefaultStandbyMode=function(profileid,callback,callbackParams){var params=new Object;if(callbackParams)params.callbackParams=callbackParams;params.profileId=profileid;params.Settings=["defaultStandby"];asyncMakeRequest("ReadProfile",params,callback)};this.GetStandbyModeSettings=function(callback){asyncMakeRequest("GetStandbyModeSettings",mapEmptyObject,
...
result.ioports.hdmi.hdcpStatus;diagResult.ioports.hdmi.status=result.ioports.hdmi.status;diagResult.ioports.hdmi.tvEdid=result.ioports.hdmi.tvEdid}diagResult.ca=new Object;if(result.ca){diagResult.ca.caCallback=result.ca.caCallback;diagResult.ca.ecmReception=result.ca.ecmReception;diagResult.ca.emmReception=result.ca.emmReception;diagResult.ca.status=result.ca.status}diagResult.sw=new Object;if(result.sw){diagResult.sw.cmVersion=result.sw.cmVersion;diagResult.sw.lastSWUpdate=result.sw.lastSWUpdate;
diagResult.sw.version=result.sw.version;if(result.sw.modules){diagResult.sw.modules=new Object;diagResult.sw.modules.nas=result.sw.modules.nas;diagResult.sw.modules.nps=result.sw.modules.nps;diagResult.sw.modules.sdk=result.sw.modules.sdk}}diagResult.transportstream=new Object;if(result.transportstream){diagResult.transportstream.lockSignal=result.transportstream.lockSignal!=null?result.transportstream.lockSignal:"";diagResult.transportstream.freq=result.transportstream.freq!=null?result.transportstream.freq:
"";diagResult.transportstream.modulation=result.transportstream.modulation!=null?result.transportstream.modulation:"";diagResult.transportstream.bitRate=result.transportstream.bitRate!=null?result.transportstream.bitRate:"";diagResult.transportstream.nid=result.transportstream.nid!=null?result.transportstream.nid:""}diagResult.freqscan=new Object;if(result.freqscan){diagResult.freqscan.tsid=result.freqscan.tsid!=null?result.freqscan.tsid:"";diagResult.freqscan.quality=result.freqscan.quality!=null?
result.freqscan.quality:0;diagResult.freqscan.level=result.freqscan.level!=null?result.freqscan.level:0}diagResult.chanstatus=new Object;if(result.chanstatus){diagResult.chanstatus.lockStatus=result.chanstatus.lockStatus!=null?result.chanstatus.lockStatus:"";diagResult.chanstatus.ber=result.chanstatus.ber!=null?result.chanstatus.ber:"";diagResult.chanstatus.freq=result.chanstatus.freq!=null?result.chanstatus.freq:"";diagResult.chanstatus.symbolRate=result.chanstatus.symbolRate!=null?result.chanstatus.symbolRate:
"";diagResult.chanstatus.modulation=result.chanstatus.modulation!=null?result.chanstatus.modulation:"";diagResult.chanstatus.snr=result.chanstatus.snr!=null?result.chanstatus.snr:"";diagResult.chanstatus.inversion=result.chanstatus.inversion!=null?result.chanstatus.inversion:"";diagResult.chanstatus.quality=result.chanstatus.quality!=null?result.chanstatus.quality:0;diagResult.chanstatus.level=result.chanstatus.level!=null?result.chanstatus.level:0}return diagResult}else return null}this.GetBootupState=
function(callback){asyncMakeRequest("GetBootupState",mapEmptyObject,callback)};this.MapGetStartUpMode=function(callback){asyncMakeRequest("MapGetStartUpMode",mapEmptyObject,callback)};this.MapSetStartUpMode=function(mode,callback){asyncMakeRequest("MapSetStartUpMode",{"mode":mode},callback)};this.RemotePairing=function(callback){asyncMakeRequest("MapRF4CBind",mapEmptyObject,callback)};this.RemoteUnbind=function(id,callback){var params=new Object;params.bindingId=id;asyncMakeRequest("MapRF4CUnbind",
params,callback)};this.RemotePairingAbort=function(callback){asyncMakeRequest("MapRF4CAbortBind",mapEmptyObject,callback)};this.GetCacheUpdateStatus=function(callback){asyncMakeRequest("AppGetCacheUpdateStatus",mapEmptyObject,callback)};this.SetCacheEventListener=function(callback,params){asyncMakeRequest("AppSetCacheEventListener",params,callback)};this.UpdateCache=function(callback){asyncMakeRequest("AppUpdateCache",mapEmptyObject,callback)};this.SwapCache=function(callback){asyncMakeRequest("AppSwapCache",
mapEmptyObject,callback)};this.AbortCaching=function(callback){asyncMakeRequest("AppAbortCaching",mapEmptyObject,callback)};this.RemoveCacheEventListener=function(callback,params){asyncMakeRequest("AppRemoveCacheEventListener",params,callback)};this.GetCADecoderStatus=function(callback){asyncMakeRequest("MapGetNagraSystemInfomation",mapEmptyObject,callback)};this.MapSwUpdateCheck=function(callback){asyncMakeRequest("MapSwCheckUpdate",mapEmptyObject,callback)};this.MapSwStartDownload=function(callback){asyncMakeRequest("MapSwStartDownload",
mapEmptyObject,callback)};this.MapSwInstall=function(callback){asyncMakeRequest("MapSwInstall",mapEmptyObject,callback)};this.MapSwReboot=function(callback){asyncMakeRequest("MapSwReboot",mapEmptyObject,callback)};this.MapSwDownloadProgress=function(callback){asyncMakeRequest("MapSwDownloadProgress",mapEmptyObject,callback)};this.MapSetAgeRating=function(newAgeRating,callback){newAgeRating=newAgeRating?newAgeRating:USER_AGE_RATING_OFF;param={};param.ageRating=newAgeRating;param.callbackParams=newAgeRating;
asyncMakeRequest("MapSetUserAgeRating",param,callback)};this.MapSwReset=function(mode,callback){param={};param.mode=mode;asyncMakeRequest("MapSwReset",param,callback)}};
...
PMRPCCall("init",urlsExpectedInInitialization,handleInitReply)};this["do"]=function(stateUrl,modelUrl){try{log.debug("WebApp: controller.do('"+stateUrl+"'"+(modelUrl?", '"+modelUrl+"'":"")+")");filterDoCommand(stateUrl,modelUrl);var params=[stateUrl,modelUrl];PMRPCCall("do",params)}catch(error){log.error("WebApp.Controller: error thrown BY WEBAPP processing do - "+error);if(error.stack)log.error(error.stack)}};function generateCallbackKey(prefix){return prefix+self.callbackCounter++}this.load=function(modelUrl,
loadNotifyCB){try{log.debug("WebApp: controller.load('"+modelUrl+"', "+(loadNotifyCB&&loadNotifyCB.name?loadNotifyCB.name:"anonymous")+")");var callbackKey=generateCallbackKey("load-callback-");self.loadCallbacks[callbackKey]=loadNotifyCB;var params=[modelUrl,callbackKey];PMRPCCall("load",params)}catch(error){log.error("WebApp.Controller: error thrown BY WEBAPP processing load - "+error);if(error.stack)log.error(error.stack)}};this.registerPMRPC=function(){pmrpc.register({publicProcedureName:"update-model",
procedure:function(param){try{log.trace("WebApp.Controller: pmrpc model update message received - "+param);if(model.initialized)self.UpdateModel(param);else log.trace("WebApp.Controller: ignoring model update "+"prior to completing initialization")}catch(error){log.error("WebApp.Controller: error thrown BY WEBAPP processing model update - "+error);if(error.stack)log.error(error.stack)}}});pmrpc.register({publicProcedureName:"load-callback",procedure:function(param){try{log.trace("WebApp.Controller: pmrpc load callback received - "+
param);self.processLoadCallback(param)}catch(error){log.error("WebApp.Controller: error processing load callback - "+error);if(error.stack)log.error(error.stack)}}});pmrpc.register({publicProcedureName:"on-callback",procedure:function(param){try{log.trace("WebApp.Controller: pmrpc 'on' callback received - "+param);self.processOnCallback(param)}catch(error){log.error("WebApp.Controller: error processing 'on' callback - "+error);if(error.stack)log.error(error.stack)}}});pmrpc.register({publicProcedureName:"off-callback",
procedure:function(param){try{log.trace("WebApp.Controller: pmrpc 'off' callback received - "+param);self.processOffCallback(param)}catch(error){log.error("WebApp.Controller: error processing 'off' callback - "+error);if(error.stack)log.error(error.stack)}}})};this.UpdateModel=function(param){try{if(param[0]==="key"){log.trace("WebApp.Controller: key input received - "+JSON.stringify(param[1]));model.state.key=param[1];notifyView("model.state.key")}else if(param[0]=="application"){var thisAppHadFocusPreviously=
model.state.applications[param[1]].inFocus;model.state.applications[param[1]].inFocus=param[2]=="true";if(model.state.applications[param[1]].inFocus)if(!thisAppHadFocusPreviously)log.trace("WebApp.Controller: app "+param[1]+" already had focus");else{removeFocusFromOtherApplications(param[1]);log.trace("WebApp.Controller: focus set on app "+param[1]);if(self.backwardCompatibilityMode)notifyView("model.state.applications."+param[1])}else log.trace("WebApp.Controller: focus unset on app "+param[1])}else if(param[0]==
"channels"){log.trace("WebApp.Controller: channel array update received");model.channels=param[1]}else if(param[0]=="applications"){log.trace("WebApp.Controller: application array update received");var previousApplicationArray=model.state.applications;model.state.applications=param[1];if(self.backwardCompatibilityMode)detectApplicationChangeEventsAndNotify(previousApplicationArray)}else if(param[0]=="players"){log.trace("WebApp.Controller: player array update received");var previousPlayerArray=model.state.players;
...
"currentProfile"){log.trace("WebApp.Controller: current profile settings update received");model.state.settings.current.profile=param[1];notifyView("model.state.settings.current.profile")}}catch(error){log.error("initWidgetModel error - "+error);if(error.stack)log.error(error.stack)}};this.processLoadCallback=function(param){try{var callbackKey=param[0];var url=param[1];if(self.loadCallbacks[callbackKey]===undefined){log.error("WebApp.Controller: could not identify callback "+"with key "+callbackKey);
return}self.loadCallbacks[callbackKey].apply(self,[url]);delete self.loadCallbacks[callbackKey]}catch(error){log.error("initWidgetModel error - "+error);if(error.stack)log.error(error.stack)}};this.processOnCallback=function(param){try{var callbackKey=param[0];var url=param[1];var data=param[2];self.storeTransmittedModelData(url,data);if(self.onCallbacks[callbackKey]===undefined){log.error("WebApp.Controller: could not identify 'on' "+"callback with key "+callbackKey);return}log.trace("WebApp.Controller: calling proxied listener for "+
url);self.onCallbacks[callbackKey].apply(self,[url])}catch(error){log.error("initWidgetModel error - "+error);if(error.stack)log.error(error.stack)}};this.storeTransmittedModelData=function(url,data){try{var javascriptModelVariable=url.replace(/(\.)([0-9]+)/g,"[$2]");var javascriptStatement=javascriptModelVariable+" = data;";eval(javascriptStatement)}catch(ex){log.warn("WebApp.Controller: Cannot evaluate transmitted model data. "+ex)}};this.processOffCallback=function(param){try{var callbackKey=param[0];
var url=param[1];if(self.offCallbacks[callbackKey]===undefined){log.error("WebApp.Controller: could not identify 'off' "+"callback with key "+callbackKey);return}self.offCallbacks[callbackKey].apply(self,[url]);delete self.offCallbacks[callbackKey]}catch(error){log.error("initWidgetModel error - "+error);if(error.stack)log.error(error.stack)}};var applicationIndex=null;this.getApplicationIndex=function(defaultValue){if(applicationIndex!=null)return applicationIndex;for(var i=0;i<model.state.applications.length;i++)if(window.name===
...
initReply.returnValue[2];model.state.system=initReply.returnValue[3];model.state.settings.current.profile=initReply.returnValue[4];model.initialized=true;log.trace("WebApp.Controller: WebApp.model is initialized");notifyView("model.initialized")}function notifyView(modelURL){try{log.debug("WebApp.Controller: notifyView('"+modelURL+"')");setTimeout(function(){emitter2.emit(modelURL,modelURL)},0)}catch(error){log.error("WebApp.Controller: error thrown by listener for event "+modelURL+" - "+error);if(error.stack)log.error(error.stack)}}
function PMRPCCall(procedure,params,successReplyHandler){if(successReplyHandler==undefined)successReplyHandler=pmrpcSuccessCallback;params.splice(0,0,window.name);log.trace("WebApp.Controller: sending pmrpc call to master controller - "+procedure+"("+JSON.stringify(params)+")");pmrpc.call({destination:window.top,publicProcedureName:procedure,params:[params],timeout:1E4,onSuccess:successReplyHandler,onError:pmrpcErrorCallback})}function pmrpcSuccessCallback(returnObject){try{log.trace("WebApp.Controller: pmrpc message ("+
returnObject.publicProcedureName+") delivery confirmed");if(returnObject.returnValue!=undefined){log.trace("WebApp.Controller: object returned from pmrpc message send:  "+JSON.stringify(returnObject.returnValue));log.warn("WebApp.Controller: this return object is ignored")}}catch(error){log.error("initWidgetModel error - "+error);if(error.stack)log.error(error.stack)}}function pmrpcErrorCallback(statusObject){try{log.error("WebApp.Controller: pmrpc message "+statusObject.publicProcedureName+"("+JSON.stringify(statusObject.params)+
") send FAILED");log.error("WebApp.Controller: error status info returned: "+statusObject.message)}catch(error){log.error("initWidgetModel error - "+error);if(error.stack)log.error(error.stack)}}function detectApplicationChangeEventsAndNotify(previousApplicationArray){if(previousApplicationArray.length==0)return;for(var app_i=0;app_i<previousApplicationArray.length;app_i++){if(objectChanged(previousApplicationArray[app_i].state,model.state.applications[app_i].state)){log.trace("WebApp.Controller: "+
describeApplicationPropertyChange(previousApplicationArray,app_i,"state")+", notifying view");notifyView("model.state.applications."+app_i+".state")}if(objectChanged(previousApplicationArray[app_i].appMsg,model.state.applications[app_i].appMsg)){log.trace("WebApp.Controller: "+describeApplicationPropertyChange(previousApplicationArray,app_i,"appMsg")+", notifying view");notifyView("model.state.applications."+app_i+".appMsg")}if(previousApplicationArray[app_i].media==undefined&&model.state.applications[app_i].media!=
undefined){if(model.state.applications[app_i].media.assets&&model.state.applications[app_i].media.assets.length>0){log.trace("WebApp.Controller: app media data was created");for(var asset_i=0;asset_i<model.state.applications[app_i].media.assets.length;asset_i++)notifyView("model.state.applications."+app_i+".media.assets."+asset_i)}}else if(previousApplicationArray[app_i].media!=undefined&&model.state.applications[app_i].media==undefined)log.warn("WebApp.Controller: "+"app media data was either deleted");
else if(objectChanged(previousApplicationArray[app_i].media.assets,model.state.applications[app_i].media.assets))if(model.state.applications[app_i].media.assets.length>previousApplicationArray[app_i].media.assets.length){log.trace("WebApp.Controller: "+"media asset list grew (from "+previousApplicationArray[app_i].media.assets.length+" to "+model.state.applications[app_i].media.assets.length+"), notifying view");notifyView("model.state.applications."+app_i+".media.assets."+(model.state.applications[app_i].media.assets.length-
1))}}}function describeApplicationPropertyChange(previousApplicationArray,app_i,propertyKey){var beforeValue=previousApplicationArray[app_i][propertyKey];var afterValue=model.state.applications[app_i][propertyKey];var beforeDescription;var afterDescription;if(beforeValue instanceof Object)beforeDescription=JSON.stringify(beforeValue).substring(0,100);else beforeDescription=beforeValue;if(afterValue instanceof Object)afterDescription=JSON.stringify(afterValue).substring(0,100);else afterDescription=
afterValue;return"model.state.applications."+app_i+"."+propertyKey+" changed from "+beforeDescription+" to "+afterDescription}function detectPlayerChangeEventsAndNotify(previousPlayerArray){if(previousPlayerArray.length==0)return;for(var player_i=0;player_i<previousPlayerArray.length;player_i++){if(objectChanged(previousPlayerArray[player_i].currentProgram,model.state.players[player_i].currentProgram)){log.trace("WebApp.Controller: "+describePlayerPropertyChange(previousPlayerArray,player_i,"currentProgram")+
", notifying view");notifyView("model.state.players."+player_i+".currentProgram")}if(objectChanged(previousPlayerArray[player_i].isBlocked,model.state.players[player_i].isBlocked)){log.trace("WebApp.Controller: "+describePlayerPropertyChange(previousPlayerArray,player_i,"isBlocked")+", notifying view");notifyView("model.state.players."+player_i+".isBlocked")}if(objectChanged(previousPlayerArray[player_i].playRate,model.state.players[player_i].playRate)){log.trace("WebApp.Controller: "+describePlayerPropertyChange(previousPlayerArray,
player_i,"playRate")+", notifying view");notifyView("model.state.players."+player_i+".playRate")}if(objectChanged(previousPlayerArray[player_i].source,model.state.players[player_i].source)){log.trace("WebApp.Controller: "+describePlayerPropertyChange(previousPlayerArray,player_i,"source")+", notifying view");notifyView("model.state.players."+player_i+".source")}if(objectChanged(previousPlayerArray[player_i].status,model.state.players[player_i].status)){log.trace("WebApp.Controller: "+describePlayerPropertyChange(previousPlayerArray,
player_i,"status")+", notifying view");notifyView("model.state.players."+player_i+".status")}if(objectChanged(previousPlayerArray[player_i].position,model.state.players[player_i].position)){log.trace("WebApp.Controller: "+describePlayerPropertyChange(previousPlayerArray,player_i,"position")+", notifying view");notifyView("model.state.players."+player_i+".position")}if(objectChanged(previousPlayerArray[player_i].availAudioLangs,model.state.players[player_i].availAudioLangs)){log.trace("WebApp.Controller: "+
describePlayerPropertyChange(previousPlayerArray,player_i,"availAudioLangs")+", notifying view");notifyView("model.state.players."+player_i+".availAudioLangs")}if(objectChanged(previousPlayerArray[player_i].audioLang,model.state.players[player_i].audioLang)){log.trace("WebApp.Controller: "+describePlayerPropertyChange(previousPlayerArray,player_i,"audioLang")+", notifying view");notifyView("model.state.players."+player_i+".audioLang")}if(objectChanged(previousPlayerArray[player_i].availSubtitleLangs,
model.state.players[player_i].availSubtitleLangs)){log.trace("WebApp.Controller: "+describePlayerPropertyChange(previousPlayerArray,player_i,"availSubtitleLangs")+", notifying view");notifyView("model.state.players."+player_i+".availSubtitleLangs")}if(objectChanged(previousPlayerArray[player_i].subtitleLang,model.state.players[player_i].subtitleLang)){log.trace("WebApp.Controller: "+describePlayerPropertyChange(previousPlayerArray,player_i,"subtitleLang")+", notifying view");notifyView("model.state.players."+
player_i+".subtitleLang")}else if(objectChanged(previousPlayerArray[player_i].subtitleLang.enable,model.state.players[player_i].subtitleLang.enable)){log.trace("WebApp.Controller: model.state.players."+player_i+".subtitleLang.enable changed from "+previousPlayerArray[player_i].subtitleLang.enable+" to "+model.state.players[player_i].subtitleLang.enable+", notifying view");notifyView("model.state.players."+player_i+".subtitleLang.enable")}if(objectChanged(previousPlayerArray[player_i].teletextLang,
model.state.players[player_i].teletextLang)){log.trace("WebApp.Controller: "+describePlayerPropertyChange(previousPlayerArray,player_i,"teletextLang")+", notifying view");notifyView("model.state.players."+player_i+".teletextLang")}if(objectChanged(previousPlayerArray[player_i].window,model.state.players[player_i].window)){log.trace("WebApp.Controller: "+describePlayerPropertyChange(previousPlayerArray,player_i,"window")+", notifying view");notifyView("model.state.players."+player_i+".window")}}}function describePlayerPropertyChange(previousPlayerArray,
player_i,propertyKey){var beforeValue=previousPlayerArray[player_i][propertyKey];var afterValue=model.state.players[player_i][propertyKey];var beforeDescription;var afterDescription;if(beforeValue instanceof Object)beforeDescription=JSON.stringify(beforeValue).substring(0,100);else beforeDescription=beforeValue;if(afterValue instanceof Object)afterDescription=JSON.stringify(afterValue).substring(0,100);else afterDescription=afterValue;return"model.state.players."+player_i+"."+propertyKey+" changed from "+
beforeDescription+" to "+afterDescription}function objectChanged(x,y){return!objectEquals(x,y)&&!bothObjectsNotNullOrUndefined(x,y)}function objectEquals(x,y){if(x instanceof Function){if(y instanceof Function)return x.toString()===y.toString();return false}if(x===null||x===undefined||y===null||y===undefined)return x===y;if(x===y||x.valueOf()===y.valueOf())return true;if(x instanceof Date)return false;if(y instanceof Date)return false;if(!(x instanceof Object))return false;if(!(y instanceof Object))return false;
var p=Object.keys(x);return Object.keys(y).every(function(i){return p.indexOf(i)!==-1})?p.every(function(i){return objectEquals(x[i],y[i])}):false}function bothObjectsNotNullOrUndefined(objA,objB){return(objA===null||objA===undefined)&&(objB===null||objB===undefined)}};
