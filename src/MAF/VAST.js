 ...
 **/
/**
 * @classdesc A VAST Parser Class that will download and parse a VAST XML document. An example app for with implementation details can be found here: <a href="https://github.com/Metrological/maf3-sdk/tree/master/apps/com.metrological.app.VASTTemplate/Contents" target="_blank">VASTTemplate</a>.
 * @class MAF.VAST
...
define('MAF.VAST', function() {
	...
	function parseCreativeLinearElement(creativeElement) {
		...
		childsByName(creativeElement, 'TrackingEvents').forEach(function(trackingEvent) {
			childsByName(trackingEvent, 'Tracking').forEach(function(tracker) {
				var event = tracker.getAttribute('event'),
					offset = tracker.getAttribute('offset'),
					trackerTemplate = parseNodeText(tracker);
		...
		if (iconsElement) {
			childsByName(iconsElement, 'Icon').forEach(function(child) {
				...
				childsByName(child, 'StaticResource').forEach(function(stat) {
					icon.type = stat.getAttribute('creativeType') || 0;
					icon.staticResource = parseNodeText(stat);
				});
	...
	function parseNonLinear(creativeElement) {
		...
		childsByName(creativeElement, 'TrackingEvents').forEach(function(trackerEvent) {
			childsByName(trackerEvent, 'Tracking').forEach(function(tracker) {
				var eventName = tracker.getAttribute('event'),
					trackingURLTemplate = parseNodeText(tracker);
		...
		childsByName(creativeElement, 'NonLinear').forEach(function(nonLinear) {
			...
			childsByName(nonLinear, 'StaticResource').forEach(function(stat) {
				nonlinearAd.type = stat.getAttribute('creativeType') || 0;
				nonlinearAd.staticResource = parseNodeText(stat);
			});
	...
	function parseCompanionAd(creativeElement) {
		...
		childsByName(creativeElement, 'Companion').forEach(function(companion) {
			...
			childsByName(companion, 'StaticResource').forEach(function(stat) {
				companionAd.type = stat.getAttribute('creativeType') || 0;
				companionAd.staticResource = parseNodeText(stat);
			});
	...
	function parseExtension(collection, extensions) {
		...
		extensions.forEach(function(extNode) {
			...
			[].forEach.call(extNode.childNodes, function(child) {
	...
	function parseInLineElement(inLineElement) {
		...
		[].forEach.call(inLineElement.childNodes, function(node) {
			...
			if (nodeName === 'Error')
				...
				childsByName(node, 'Creative').forEach(function(creative) {
					[].forEach.call(creative.childNodes, function(child) {
						var adCreative;
	...
	function parseAdElement(adElement) {
		...
		[].forEach.call(adElement.childNodes, function(node) {
		...
		try {
			request = new Request({
				...
				onSuccess: function(xhr) {
					if (getSetting('screenDebug')) screen.log('VAST SUCCESS: ' + xhr);;
					cb(null, xhr);
				},
				onError: function(err) {
					if (getSetting('screenDebug')) screen.log('VAST Error: ' + err);
					cb(null, err);
				},
				onFailure: function(fail) {
					if (getSetting('screenDebug')) screen.log('VAST Fail: ' + fail);
					cb(null, fail);
				},
				onTimeout: function(to) {
					if (getSetting('screenDebug')) screen.log('VAST TimeOut: ' + to);
					cb(null, to);
				}
			});
			...
			if (getSetting('screenDebug')) screen.log('VAST: Unexpected network error');
			cb(new Error('Unexpected network error.'));
		}
	}
	...
	function parse(url, baseURLS, options, cb) {
		...
		get(url, options, function(err, xml) {
			...
			if (getSetting('screenDebug')) screen.log('CHECK: ' + xml + ' - ' + xml.documentElement + ' - ' + xml.documentElement.nodeName + ' - ' + xml.documentElement.childNodes + ' - ' + xml.documentElement.childNodes.length);
			...
			[].forEach.call(xml.documentElement.childNodes, function(node) {
				var ad;
	...
	return new MAF.Class({
		...
		Protected: {
			...
			get: function(url, cb) {
				...
				if (widget.getSetting('ads') === false)
					cb(new Error('Ads are disabled'), null);
				else
			...
			Tracker: new MAF.Class({
				Protected: {
					 ...
					 * Note: Clicking is not possible on a Settop Box. This is just here for spec compliance.
				...
				initialize: function(ad, creative) {
					...
					if (this.creative.type && this.creative.type === 'linear' && Object.contains(this.creative, 'duration', true) && Object.contains(this.creative, 'skipDelay', true)) {
